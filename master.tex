...
\begin{frame}
  \makebox[\linewidth]{
    \begin{tabular}{cccc}
      \includegraphics[height=18mm]{cherrypy_logo.png}
      \hspace{14pt}
      \includegraphics[height=18mm]{nagare_logo.png} &
      \includegraphics[height=18mm]{albatross_logo.png} &
      \includegraphics[height=18mm]{grok_logo.png} &
      \includegraphics[width=18mm]{webware_logo.png} \\[4pt]

      \includegraphics[width=24mm]{bottle_logo.png} &
      \includegraphics[width=28mm]{django-logo.png} &
      \includegraphics[width=14mm]{pylons_logo.png} &
      \includegraphics[width=25mm]{webpy_logo.png} \\[4pt]
      ...
      \includegraphics[width=16mm]{pyjs_logo.png} &
      \includegraphics[height=16mm]{spyce_logo.png} &
      \includegraphics[width=16mm]{tg.png} &
      \includegraphics[width=16mm]{zope_logo.png} \\
    \end{tabular}
  }
...
\begin{frame}[fragile]
  ...
  \begin{tikzpicture}[overlay]
    \node at (30mm, 28mm)
      {\includegraphics[width=63mm]{python_presentation.png}};
  \end{tikzpicture}\source{[\ref{illu:13}]}
  \end{figure}
...
\begin{frame}[fragile]
  \frametitle{Kernphilosophie~-- \textit{The Zen of Python}}
  ...
  \begin{beamerpostit}
    ...
    \dots ”
    \end{quote}
  \end{beamerpostit}\source{[\ref{zit:5}]}
    \end{column}
    \begin{column}{.3\textwidth}<1->
      \includegraphics[width=.9\textwidth]{zen.png}
      \source{[\ref{illu:11}]}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}
  ...
  %~ Argumentation warum das für Webanwendungen besonders geeignet ist
  \begin{itemize}[<1->]
...
\begin{frame}
  \frametitle{Turbogears}
  %~ http://www.turbogears.org/2.2/docs/main/TG2Philosophy.html

  \vspace{-32pt}
  \begin{figure}
            \includegraphics[width=20mm]{tg.png}
            \source{[\ref{illu:10}]}
  \end{figure}
  \vspace{-18pt}
  ...
  \begin{tikzpicture}[scale=0.88] % timeline 2003-2012->
    % define coordinates (begin, used, end, arrow)
    \foreach \x in {2005,2009,2011,2012,2013}{
        \pgfmathsetlength\yearposx{(\x-2005)*1cm};
        \coordinate (y\x)   at (\yearposx,0);
        \coordinate (y\x t) at (\yearposx,+3pt);
        \coordinate (y\x b) at (\yearposx,-3pt);
    }
    ...
    % draw ticks
    \foreach \x in {2009,2011,2012}
        \draw (y\x t) -- (y\x b);

    % annotate (draw nodes)
    \foreach \x in {2005,2009,2011,2012}
        \node at (y\x) [below=3pt] {\x};

    \fill (y2005) circle (3pt) node[above=12pt] {1.0};
    \fill (y2009) circle (3pt) node[above=13pt] {1.1};
    \fill (y2009) circle (3pt) node[above=3pt] {2.0};
    \fill (y2011) circle (3pt) node[above=13pt] {1.5};
    \fill (y2011) circle (3pt) node[above=3pt] {2.1};
    \fill (y2012) circle (3pt) node[above=3pt] {2.2};
  \end{tikzpicture}
  \vspace{12pt} \\

  \begin{itemize}[<1->]
  \item Aus extern gepflegten Python-Modulen zusammengesetzt
  \item Module sind austauschbar
  \end{itemize}
...
\begin{frame}
  \frametitle{Persistenz}
    ...
    \begin{itemize}[<1->]
        ...
        \item SQLAlchemy (Pylons, Turbogears 2) %Gork,Zope
        % \item Storm (Grok )
...
<p><label for="id_name">Name:</label> <input id="id_name" type="text" name="name" maxlength="60" /></p>
<p><label for="id_boss">Boss:</label> <input type="checkbox" name="boss" id="id_boss" /></p>
<p><label for="id_concern">Concern:</label> <select name="concern" id="id_concern"><option value="" selected="selected">---------</option><option value="1">Concern object</option>
</select></p>  <div style='display:none'><input type='hidden' name='csrfmiddlewaretoken' value='mgPxkm0c21WH81X0LIZEFoBxrDhCY2oW' /></div>
  <input type="submit" value="Submit" />
</form>
  \end{lstlisting}
...
\begin{frame}[fragile]
  ...
  \framesubtitle{Django}
  \begin{itemize}[<1->]
    \item Unterstützt Textübersetzung, Datums-, Zeit und Zahlenformatierung,
        Zeitzonen und Pluralisierung
    \item Markieren der Texte mit \textit{ugettext}
    \item Übersetzung innerhalb von Python, Templates und JavaScript
  \end{itemize}
  \begin{lstlisting}
 ...
 \rowcolors{1}{white}{solarized@base3}
    \begin{table}[h]
        \begin{tabular}{|c|c|c|}
            ...
            %~ Scaffolding: “Django has a bit of a different philosophy. It
            %~ gives you tools to make doing things easily but doesn't actually
            %~ do it for you (except the admin interface).”
            %~ => no boilerplate code
            ...
            %~ http://turbogears.org/2.2/docs/main/Testing/
            Unit testing  & \checkmark \cellcolor{dkgreen} & nose \cellcolor{dkgreen} \\ \hline
            %~ http://turbogears.org/2.2/docs/main/Caching.html
...
\begin{beamerpostit}
    ...
    \end{quote} %~ FIXME Quelle
  \end{beamerpostit}\source{[\ref{zit:1}]}
...
\begin{frame}
 ...
 \begin{column}{.6\textwidth}<1->
  ...
  \begin{beamerpostit}
    ...
    \end{quote} %~ FIXME Quelle
  \end{beamerpostit}\source{[\ref{zit:2}]}
  \end{column}
  \end{columns}
...
\begin{itemize}[<1->]
                \item Core Web-Framework
                    \begin{itemize}[<1->]
                    \item Template System (tornado.template)
                    \item Internationalisierung (tornado.locale)
                    \item Unit Testing (tornado.testing)
                \end{itemize}
                \item Einige weitere Hilfsmittel, Services, \dots
                \begin{itemize}[<1->]
                    \item Nicht blockierender HTTP-Client
                    \item Login mit OpenID und OAuth (3rd party)
...
\begin{frame}
    \begin{columns}[T]<1->
        ...
        \begin{beamerpostit}
                \begin{quote}
                    “\dots a fast, simple and lightweight WSGI micro web-framework for Python \dots"
                \end{quote}
            \end{beamerpostit}\source{[\ref{zit:3}]}
            ...
            \begin{itemize}[<1->]
                    ...
                    \item Keine Abhängigkeiten (alles in einem Modul)
...
\begin{frame}
    ...
    \begin{columns}[T]<1->
        ...
        \end{columns}
         \begin{beamerpostit}
            \begin{quote}
      “\dots is a web framework for Python that is as simple as it is powerful \dots”
    \end{quote} %~ FIXME Quelle
         \end{beamerpostit}\source{[\ref{zit:4}]}

         \vspace{20px}
            \begin{itemize}[<1->]
                \item Was kann/ist web.py ?
            \begin{itemize}[<1->]
                    \item Eigene Template Sprache (angelehnt an Python)
                    \item Eingebaute Formularunterstützung
                    \item Abstraktion der Datenbank API ohne ORM
                \end{itemize}
        \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}[<1->]
    \item Je nach Vorliebe, Anforderungen und Vorkenntnissen
    \item Jedes Framework hat Stärken/Schwächen
  \end{itemize}

  \begin{figure}
  \begin{tikzpicture}[overlay]
    \node at (30mm, -30mm)
      {\includegraphics[width=63mm]{python_presentation_end.png}};
  \end{tikzpicture}\source{[\ref{illu:14}]}
  \end{figure}
